  ...
  useEffect(() => {
    const measure = () => {
      if (canvasContainerRef.current) {
        ...
        const w = rect.width || canvasContainerRef.current.offsetWidth || canvasContainerRef.current.clientWidth;
        if (w > 10) {
          setCanvasDisplayWidth(Math.min(CANVAS_WIDTH, Math.floor(w)));
        }
      }
    };
    // Multiple attempts to catch layout at different stages
    measure();

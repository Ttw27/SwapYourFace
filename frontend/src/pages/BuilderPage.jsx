...
const DraggableText = ({ text, x, y, fontSize, fill, stroke, strokeWidth, isSelected, onSelect, onChange }) => {
  ...
  const trRef = useRef();
  useEffect(() => {
    if (isSelected && trRef.current && hitRef.current) {
  ...
  return (
    <>
      {strokeWidth > 0 && (
        <Text text={text.toUpperCase()} x={x} y={y} fontSize={fontSize} fontFamily="Anton, Impact, sans-serif"
          fill={stroke} stroke={stroke} strokeWidth={strokeWidth * 2.5}
          align="center" width={CANVAS_WIDTH} offsetX={CANVAS_WIDTH / 2} listening={false} />
      )}
      <Text text={text.toUpperCase()} x={x} y={y} fontSize={fontSize} fontFamily="Anton, Impact, sans-serif"
        fill={fill} align="center" width={CANVAS_WIDTH} offsetX={CANVAS_WIDTH / 2} listening={false} />
      <Text ref={hitRef} text={text.toUpperCase()} x={x} y={y} fontSize={fontSize} fontFamily="Anton, Impact, sans-serif"
        fill="rgba(0,0,0,0)" align="center" width={CANVAS_WIDTH} offsetX={CANVAS_WIDTH / 2}
        draggable onClick={onSelect} onTap={onSelect}
        onDragEnd={(e) => onChange({ x: e.target.x(), y: e.target.y() })}
        onTransformEnd={() => { const n = hitRef.current; onChange({ x: n.x(), y: n.y(), fontSize: Math.round(fontSize * n.scaleX()) }); n.scaleX(1); n.scaleY(1); }}
      />
      {isSelected && <Transformer ref={trRef} enabledAnchors={['middle-left','middle-right']} boundBoxFunc={(o,n) => n.width < 50 ? o : n} />}
    </>
  );
};

// ─── Head Image with touch ────────────────────────────────────────────────────
  ...
  return (
    <>
      <KonvaImage ref={shapeRef} image={image}
        x={placement.x * CANVAS_WIDTH} y={placement.y * CANVAS_HEIGHT}
        width={image.width * placement.scale * 0.3} height={image.height * placement.scale * 0.3}
        offsetX={(image.width * placement.scale * 0.3) / 2} offsetY={(image.height * placement.scale * 0.3) / 2}
        rotation={placement.rotation} draggable
        onClick={onSelect} onTap={onSelect}
        onDragEnd={(e) => onChange({ x: e.target.x() / CANVAS_WIDTH, y: e.target.y() / CANVAS_HEIGHT })}
        onTransformEnd={() => { const n = shapeRef.current; onChange({ scale: placement.scale * n.scaleX(), rotation: n.rotation() }); n.scaleX(1); n.scaleY(1); }}
      />
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="order-2 lg:order-1">
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              <div className="rounded-xl overflow-hidden border border-gray-100" style={{ background: currentShirtColor.hex, touchAction:'none', transition:'background 0.2s' }}>
                <Stage
                  ref={stageRef}
                  width={CANVAS_WIDTH}

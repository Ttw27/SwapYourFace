...
export default function GalleryPage() {
  ...
  const filteredTemplates = templates.filter(template => {
    ...
    const matchesFilter =
      filter === 'all' ||
      (filter === 'stag' && cats.includes('stag')) ||
      (filter === 'hen' && cats.includes('hen')) ||
      (filter === 'party' && cats.includes('party')) ||
  ...
  const filterOptions = [
    ...
    { value: 'party', label: 'Party', icon: null },
    ...
    <div className="min-h-screen bg-[#F7F7F7]">
      <section className="bg-white py-12 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <h1 className="font-['Anton'] text-4xl sm:text-5xl text-[#252A34] mb-4 tracking-wide">TEMPLATE GALLERY</h1>
            <p className="text-gray-600 max-w-2xl mx-auto">Choose from our collection of hilarious body templates for your party t-shirts</p>
          </motion.div>
        </div>
      </section>
            ...
            <div className="flex gap-2 flex-wrap justify-center">
              {filterOptions.map((option) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${filter === option.value ? 'bg-[#FF2E63] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredTemplates.map((template, index) => (
                <motion.div
                  ...
                  className="group"
                >
                  <div className="card-party p-4 h-full flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100">
                    <div className="relative aspect-square bg-gray-50 rounded-xl overflow-hidden mb-4">
                      {/* Show product_image_url (sample with head+text) — fall back to body_image_url */}
                      <img
                      ...
                      <div className="absolute top-3 left-3 flex gap-1 flex-wrap">
                        ...
                        {template.is_popular && (
                          <span className="px-2 py-0.5 bg-[#FF2E63] text-white text-xs font-bold rounded-full uppercase">Popular</span>
                        )}
                        {template.is_new && (
                          <span className="px-2 py-0.5 bg-[#08D9D6] text-[#252A34] text-xs font-bold rounded-full uppercase">New</span>
                        )}
                      </div>
                    </div>
                    <h3 className="font-['Anton'] text-lg text-[#252A34] mb-1 tracking-wide">{template.name}</h3>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <h2 className="font-['Anton'] text-3xl text-[#252A34] mb-4 tracking-wide">CAN'T FIND WHAT YOU'RE LOOKING FOR?</h2>
          <p className="text-gray-600 mb-6">Contact us for custom template requests or bulk order inquiries</p>
          <Link to="/contact">
            <Button variant="outline" className="border-2 border-[#252A34] text-[#252A34] hover:bg-[#252A34] hover:text-white rounded-full px-8 py-4 font-bold uppercase tracking-wider">
              Contact Us
            </Button>
          </Link>
        </div>
